...
boop <- rbind(
  ...
  data.frame(Parameter = "$\\sigma_{obs}$", `Prior Distribution` = "Normal $\\sim (0.2, 0.2)$"),
  ...
  data.frame(Parameter = "$m$", `Prior Distribution` = "Normal $\\sim (0.25, 0.1)$"), 
  data.frame(Parameter = "$\\gamma$", `Prior Distribution` = "Exponential $\\sim 2.3$")
)
...
if(!file.exists(here("results","draws_btemp_train.rds"))) {
  if(run_in_parallel==TRUE) {
    library(foreach)
    library(doParallel)
    ...
    draws_btemp_train <- foreach(i = patches, .combine = bind_rows, .packages = c("brms","dplyr")) %dopar% 
      
      {
        
        dat_bind <- dat_catchonly %>% 
          select(btemp, year, lat) |> 
          mutate(lat_floor = floor(lat)) |> 
          filter(lat_floor == i)
        
        brm_tmp <- brm(btemp ~ year, data = dat_bind, family = gaussian(), chains = 4, iter = 2000) 
        brm_tmp_draws <- as_draws_df(brm_tmp)
        
        brm_tmp_draws |> 
          select(b_year, .chain, .iteration, .draw) |> 
          mutate(Patch = i)
        
      }
    write_rds(draws_btemp_train, here("results", "draws_btemp_train.rds"))
    stopCluster(cl)
    
  }
} else { draws_btemp_train <- read_rds(here("results","draws_btemp_train.rds")) }

btemp.train.tbl <- draws_btemp_train |> 
  group_by(Patch) |> 
  summarise(
    Median = signif(median(b_year), 3),
    HPDI_lower = {
      h <- coda::HPDinterval(coda::as.mcmc(b_year), 0.95)
      h[1]
    },
    HPDI_upper = {
      h <- coda::HPDinterval(coda::as.mcmc(b_year), 0.95)
  ...
  mutate(HPDI = paste0(signif(HPDI_lower, 3), " — ", signif(HPDI_upper, 3)), .keep="unused")


kbl(btemp.train.tbl, booktabs = TRUE, caption = "Bayesian linear regressions of sea bottom temperature (measured in the trawl surveys) on year within each patch during the model training interval (1972-2006) with Gaussian errors, summarized by the median and 95\\% Highest Posterior Density Interval (HPDI) of the posterior estimate for the slope term. Values are truncated at three significant digits.",
    escape = FALSE) %>%
  kable_styling(font_size = 8, latex_options = c("striped"), full_width=TRUE)
```
...
if(!file.exists(here("results","draws_btemp_test.rds"))) {
  if(run_in_parallel==TRUE) {
    library(foreach)
    library(doParallel)
    ...
    draws_btemp_test <- foreach(i = patches, .combine = bind_rows, .packages = c("brms","dplyr")) %dopar% 
      
      {
        
        dat_bind <- dat_test_catchonly %>% 
          select(btemp, year, lat) |> 
          mutate(lat_floor = floor(lat)) |> 
          filter(lat_floor == i)
    ...
    write_rds(draws_btemp_test, here("results", "draws_btemp_test.rds"))
    stopCluster(cl)
    
  }
} else { draws_btemp_test <- read_rds(here("results","draws_btemp_test.rds")) }

btemp.test.tbl <- draws_btemp_test |> 
  group_by(Patch) |> 
  summarise(
    Median = signif(median(b_year), 3),
    HPDI_lower = {
...
kbl(btemp.test.tbl, booktabs = TRUE, caption = "Bayesian linear regressions of sea bottom temperature (measured in the trawl surveys) on year within each patch during the model testing interval (2007-2016) with Gaussian errors, summarized by the median and 95\\% Highest Posterior Density Interval (HPDI) of the posterior estimate for the slope term. Values are truncated at three significant digits.",
    escape = FALSE) %>%
  kable_styling(font_size = 8, latex_options = c("striped"), full_width=TRUE)
```
...
density_hat_50 <- density_hat_thin |>
  arrange(year) |>
  group_by(year, patch) |>
  group_split(.keep=TRUE) |> 
  lapply(\(x) {
    ci <- coda::HPDinterval(coda::as.mcmc(x$density_hat),
                            prob = .50)
    tibble(
      lower = ci[1], 
      upper = ci[2],
      year = unique(x$year),
      patch = unique(x$patch)
    )
  }) |>
  bind_rows() |> 
  mutate(ci_level = "50")

density_hat_80 <- density_hat_thin |>
  arrange(year) |>
  group_by(year, patch) |>
  group_split(.keep=TRUE) |> 
  lapply(\(x) {
...
density_hat_95 <- density_hat_thin |>
  arrange(year) |>
  group_by(year, patch) |>
  group_split(.keep=TRUE) |> 
  lapply(\(x) {
...
density_hat_ribbons <- bind_rows(density_hat_50, density_hat_80, density_hat_95) |> 
  mutate(ci_level = factor(ci_level, levels=c("95","80","50"))) # for plotting

density_hat_median <- density_hat_thin |> 
  group_by(patch, year) |> 
  summarise(median = median(density_hat))

plot_out <- density_hat_thin %>% 
  ggplot() + 
  geom_ribbon(data = density_hat_ribbons, aes(x=year, ymin=lower, ymax=upper, fill=ci_level), alpha = 0.7) + 
  geom_line(data = density_hat_median, aes(x=year, y=median)) +
...
```{r cite-packages, message=FALSE, warning=FALSE}
